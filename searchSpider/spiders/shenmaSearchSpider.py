...
class ShenmaSearchSpider(scrapy.spiders.Spider):
    name = 'shenmaSearch'
    # allow_domains = ['uodoo.com']
    ...
    def __init__(self, keyword, filters='', blackWords='', whiteWords='', blackURLs='', whiteURLs='', targetUrl=None,
        ...
        if isinstance(keyword, str):
            ...
            self.closed(u'传入keyword参数不合法！无法初始化神马爬虫')
        if not keywords:
            self.closed(u'传入keyword参数不合法！无法初始化神马爬虫')
        ...
        if len(self.filters) < 1:
            self.closed(u'传入filters参数为空！无法初始化神马爬虫')
        logging.info(u"keywods is : %s" % keywords)
        logging.info(u"神马爬虫初始化成功")
        self.isLinux = False
        super(ShenmaSearchSpider, self).__init__(*args, **kwargs)
        ...
        # "http://m.sm.cn/s?q=妹子&page=2&by=submit&safe=1&from=smor&safe=1"
        self.baseURL = ['http://m.sm.cn/s?q=', '', '&page=', '1', '&by=submit&safe=1&from=smor&safe=1']
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            results = response.xpath("//body/div[@class='article ali_row']")
            for result in results:
                item = SearchspiderItem()
                item['platform'] = u"神马搜索"
                item['keyword'] = response.meta['keyword']
                item['resultUrl'] = response.meta['url']
                item['targetUrl'] = self.getUnicode(result.xpath("h2/a[@href]/@href").extract()[0])
                item['targetTitle'] = self.getUnicode(''.join(result.xpath("h2/a[@href]//text()").extract()))
    ...
    def closed(self, reason):
        if reason:
            logging.warning(u"神马爬虫运行结束： %s" % reason)
        ...
        logging.info(u'shenmaSearchSpider 爬了这么长时间： %s' % (datetime.datetime.now() - self.startTime))
