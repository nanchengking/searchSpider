...
class CookieSpider(scrapy.Spider):
    ...
    def __init__(self, times=5):
        ...
        self.init_db()
        logging.debug(u"cookies爬虫初始化成功")

    def init_db(self):
        self.conn = sqlite3.connect("cookies.db")
        self.cursor = self.conn.cursor()
        create = "create table if not exists xiami(id integer primary key autoincrement,key text,value text)"
        delete = "delete from xiami"

        self.cursor.execute(create)
        self.cursor.execute(delete)
        self.conn.commit()
        logging.debug(u"数据库准备完毕")
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            for i in cookies:
                self.cursor.execute("select * from xiami where key =?", (i,))
                res = self.cursor.fetchone()
                if res:
                    self.cursor.execute("update xiami set value = ? where key =?", (cookies[i], i))
                else:
                    self.cursor.execute("insert into xiami(key,value)  values(?,?)", (i, cookies[i]))
                self.conn.commit()
            logging.debug(u"接受一次cookie更新")
            self.conn.commit()
            if self.counts < self.times:
                self.counts += 1
                yield Request(url=self.baseURL + str(self.counts))
        else:
            logging.debug(u"返回错误%s" % response.code)
